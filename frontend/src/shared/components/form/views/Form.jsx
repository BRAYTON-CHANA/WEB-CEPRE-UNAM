...
const Form = ({ 
  ...
  const prepareSubmitData = useCallback(() => {
    ...
    fields.forEach(field => {
      ...
      // Solo sobrescribir si el valor actual es nulo o vacío (para no sobrescribir campos duplicados con el mismo name)

      if (isHidden) {

        const currentValue = submitData[field.name];

        // Solo asignar valor si el valor actual es nulo, undefined o vacío
  ...
  const renderField = useCallback((field) => {
    ...
    const fieldKey = field.id || `${field.name}-${field.type}`;
  ...
  const renderSection = (section) => {
    ...
    const allFieldsHidden = section.fields.every(field =>
  ...
  const renderMultiStepNavigation = (part = 'full') => {

    if (!isMultiStep) return null;

    return <MultiStepNavigator {...multiStepNavProps} part={part} />;
  ...
  const renderSinglePageSubmit = () => {
    ...
    return (
      ...
      <div className={submitWrapperClassName || 'mt-6'}>
        ...
        <button
          ...
          className={submitClassName || 'w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors'}
  ...
  return (
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className={className} noValidate>
        ...
        {/* Header multi-step: progreso + título + dots */}

        {isMultiStep && renderMultiStepNavigation('header')}
        ...
        {/* Footer: botones prev/next o submit single-page */}

        {isMultiStep ? renderMultiStepNavigation('footer') : renderSinglePageSubmit()}

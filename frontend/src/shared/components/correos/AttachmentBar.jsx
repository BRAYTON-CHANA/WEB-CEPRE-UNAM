...
const AttachmentBar = ({ adjuntos, onRemove, onFilesSelected, fileInputRef }) => {
  ...
  return (
    <div
      className={`px-3 py-2 transition-colors ${dragOver ? 'bg-blue-50/80 ring-2 ring-inset ring-blue-400' : 'bg-gray-50/70'}`}
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        {adjuntos.length > 0 && (
          <button type="button" onClick={() => fileInputRef.current?.click()} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-blue-700 bg-blue-50 rounded-lg hover:bg-blue-100 cursor-pointer transition-colors">
      ...
      {adjuntos.length === 0 ? (
        <p className="text-xs text-slate-400">Sin adjuntos. Usa el clip de la barra o arrastra archivos al editor.</p>
      ) : (

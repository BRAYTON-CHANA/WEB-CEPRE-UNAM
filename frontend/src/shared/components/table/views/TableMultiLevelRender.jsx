...
const TableMultiLevelRender = ({ data, levelConfigs }) => {
  ...
  const maxDataColumns = useMemo(() => {
    ...
    const maxHeaders = Math.max(
      ...levelConfigs.map(config => {
        return (config.headers?.filter(h => h?.title) || []).length;
      })
    );
      ...
      <React.Fragment key={group.key}>
        ...
        <tr className="bg-slate-50 hover:bg-slate-100/80 border-b border-slate-200/60 transition-colors duration-150">
          {isLeafGroup ? (
            ...
            /* Grupo con hijos: el header con groupBy:true lleva el botón expand, el resto son celdas normales */
            <>
              {group.config.headers?.map((header, idx) => {
                const isGroupByCol = header.groupBy === true || header.title === group.field;
                if (isGroupByCol) {
                  return (
                    <td key={header.title} className="px-5 py-3.5 whitespace-nowrap text-sm text-gray-700" style={{ paddingLeft: `${indent + 20}px` }}>
              ...
              {(() => {
                const totalCols = group.config.headers?.length || 0;
                  ...
                  return (
                    <>
                      <th className="px-5 py-2.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider" style={{ paddingLeft: `${indent + 44}px` }}>
                        {headers[0]?.label || headers[0]?.title}
                      </th>
  ...
  const getMainHeaders = () => {
    return levelConfigs[0]?.headers?.filter(h => h?.title) || [];
  };

  const mainHeaders = getMainHeaders();
  const firstLevelHasHeaders = mainHeaders.length > 0;

...
const TestDatabaseView = () => {
  ...
  const handleSelectTable = async (tableName) => {
    ...
    try {
      console.log(`[DEBUG] 🔍 Cargando datos de tabla: ${tableName}`);
      const response = await getTableData(tableName);
      console.log(`[DEBUG] ✅ Respuesta CRUDA de getTableData:`, JSON.stringify(response, null, 2));
      console.log(`[DEBUG] ✅ Tipo de response:`, typeof response, Array.isArray(response));

      // getTableData retorna array directo de Supabase o { success, data }
      const records = Array.isArray(response) ? response : (response.data || []);
      console.log(`[DEBUG] 📊 Registros extraídos (${records.length}):`, records);
        ...
        <div style={{ marginBottom: '2rem', padding: '1rem', backgroundColor: '#fff3e0', borderRadius: '8px' }}>
          ...
          {tableStats && (
            <div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: '#f3e5f5', borderRadius: '4px' }}>
              ...
              {/* getTableStats retorna { data: { count } } */}
              <p><strong>Total de registros:</strong> {tableStats.data?.count || 0}</p>
            </div>
          )}
...
export default TestDatabaseView;

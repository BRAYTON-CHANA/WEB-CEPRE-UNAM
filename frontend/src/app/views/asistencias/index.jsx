      ...
      >
        ...
        <div className="px-6 py-12 flex flex-col items-center text-center">
          ...
          <p className="text-gray-400 text-sm max-w-xs">
            Aquí se cargará el formulario para marcar la asistencia del docente en esta sesión.
          </p>
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="border-b-2 border-gray-100">
            {headers.map(h => (
              <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap bg-white first:rounded-tl-none last:rounded-tr-none">
                {h}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {sesiones.map((s, idx) => (
            ...
            >
              ...
              <td className="px-4 py-3.5 whitespace-nowrap">
                <span className="font-mono text-gray-600 text-sm tracking-tight">{formatHora(s.HORA_INICIO)}<span className="text-gray-300 mx-1">–</span>{formatHora(s.HORA_FIN)}</span>
              </td>
              <td className="px-4 py-3.5 whitespace-nowrap">
                <span className="text-gray-500 text-sm">{s.DURACION_TOTAL_MINUTOS ? `${s.DURACION_TOTAL_MINUTOS} min` : '—'}</span>
              </td>
              <td className="px-4 py-3.5 whitespace-nowrap">
                <EstadoSesionBadge estado={s.ESTADO} />
              </td>
              <td className="px-4 py-3.5 whitespace-nowrap">
                <span className="text-gray-700 text-sm">{s.DOCENTE_PROGRAMADO_NOMBRE ?? <span className="text-gray-300 italic">Sin asignar</span>}</span>
              </td>
              <td className="px-4 py-3.5 whitespace-nowrap">
              ...
              <td className="px-4 py-3.5 max-w-[180px]">
                <span className="text-gray-400 text-sm truncate block" title={s.OBSERVACIONES ?? ''}>{s.OBSERVACIONES ?? '—'}</span>
              </td>
              <td className="px-4 py-3.5 whitespace-nowrap">
                <button
                  onClick={() => onMarcar(s)}
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-2 shadow-sm flex gap-2 flex-wrap">
              {cursos.map(c => (
                ...
                >
                  ...
                  <span className={`text-xs mt-0.5 leading-tight ${cursoActivo === c.ID_CURSO ? 'text-blue-200' : 'text-gray-400'}`}>
                    {c.NOMBRE_AREA}
                  </span>
                </button>
              ))}
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    <div className="h-1.5 w-28 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.round((totalAsistio / sesionesDelCurso.length) * 100)}%` }}
                      />
                    </div>
                    <span className="text-xs font-semibold text-gray-500">
                      {Math.round((totalAsistio / sesionesDelCurso.length) * 100)}% asistencia
                    </span>
                  </div>
                )}
              </div>
      ...
      <div className="min-h-screen py-10" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)' }}>
        <div className="max-w-screen-2xl mx-auto px-6">
          ...
          <div className="mb-8 flex items-end justify-between gap-4 flex-wrap">
            <div>
              <p className="text-xs font-semibold text-blue-500 uppercase tracking-widest mb-1">Panel de control</p>
              <h1 className="text-4xl font-black text-gray-900 tracking-tight">Asistencias</h1>
              <p className="text-gray-400 mt-1 text-sm">Registro y control de asistencia de docentes por sesión</p>
            </div>

...
import { permisosFormFields, permisosValidation, permisosModalConfig } from '@/features/permisos/config/formConfig';
import { headerProps, getHeaderActions } from '@/features/permisos/config/headerConfig';

/**
 * Configuración de PERMISOS
 * CRUD completo para la tabla PERMISOS usando CrudMultiLevelManager con un solo nivel.
 */
function PermisosConfig() {
  const { records, loading, error, refresh } = useTableData(tableConfig.tableName);

  const permisosCrud = useCrudForms({
    tableName: 'PERMISOS',
    primaryKey: 'ID_PERMISO',
    onRefresh: refresh
  });

  const tableLevelConfigs = getTableLevelConfigs(permisosCrud);
  ...
  const crudLevels = [
    {
      ...
      formLayout: null,
      validation: permisosValidation,
      confirmSubmit: true,
      modalConfig: permisosModalConfig
    }
  ];
          ...
          return (
            <div className="px-8 py-8 space-y-8 pb-12">
              <CrudHeader
                ...
                actions={getHeaderActions(permisosCrud)}
              />

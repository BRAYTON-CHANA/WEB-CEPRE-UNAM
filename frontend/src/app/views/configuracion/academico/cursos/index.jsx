...
function CursosConfig() {
  ...
  const handleSaveSuccess = useCallback((recordId, field, newValue) => {
    setTableRecords(prev =>
      prev.map(row => String(row.ID_CURSO) === String(recordId) ? { ...row, [field]: newValue } : row)
    );
  }, []);
              ...
              {!loading && !error && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
                  <TableMultiLevelEditable
                    ...
                    formatToastMessage={(recordId, field, newValue, primaryKey, rowData, header) => `${rowData?.NOMBRE_CURSO || 'Curso'}: ${header?.label || field} → ${newValue ? 'Activo' : 'No activo'}`}

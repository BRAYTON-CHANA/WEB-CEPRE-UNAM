...
import { useRequisitosDocentes } from '@/features/requisitos_docentes/hooks/useRequisitosDocentes';
...
function RequisitosDocentesConfig() {
  const {
    tableRecords, loading, error, handleSaveSuccess, tableLevelConfigs,
    isCreateOpen, dynamicFormFields, handleCloseCreate, handleCreate, handleSuccess, handleError,
    isEditOpen, selectedRecord, editFunctionWrapper, handleCloseEdit,
    isDeleteOpen, rowToDelete, deleteLoading, handleCancelDelete, handleConfirmDelete,
    notification, closeNotification,
    requisitosFormFields, requisitosMultiStep, requisitosValidation, requisitosModalConfig, createRequisito
  } = useRequisitosDocentes();
  ...
  return (
    <ConfigLayout>
      ...
      {/* Modal Crear */}
      <Modal isOpen={isCreateOpen} onClose={handleCloseCreate} title={requisitosModalConfig.createTitle} size="lg" closeOnOutsideClick={false}>
        <div className="p-6">
          <CrudForm
            key={`create-requisitos-${dynamicFormFields.find(f => f.name === 'CONDICION_LABORAL')?.defaultValue || 'global'}`}
      ...
      {/* Modal Editar */}
      <Modal isOpen={isEditOpen} onClose={handleCloseEdit} title={requisitosModalConfig.editTitle} size="lg" closeOnOutsideClick={false}>
        <div className="p-6">
          {selectedRecord && (
            <CrudForm

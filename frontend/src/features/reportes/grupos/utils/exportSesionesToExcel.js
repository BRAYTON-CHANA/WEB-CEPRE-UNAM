...
export const exportSesionesToExcel = async (idGrupo, grupoNombre, opts = {}) => {
  try {
    ...
    // 1 query JOIN: turno + bloques (idTurno viene de VW_GRUPOS vía opts,
    // fallback al ID_TURNO que trae la propia vista de sesiones)
    const idTurno = opts.idTurno ?? sesiones.find(s => s.ID_TURNO != null)?.ID_TURNO;
    if (idTurno == null) {
      alert('No se encontró el turno del grupo');
      return;
    }

    const turnosMap = await fetchTurnosConBloques([idTurno]);
    const customBlocks = turnosMap.get(idTurno)?.bloques || [];
    ...
    titleCell.value = `HORARIO - ${nombreSede} - ${grupoNombre || 'Grupo'}`;
...
export const exportAllSesionesToExcel = async (grupos, opts = {}) => {
  try {
    ...
    for (const grupo of grupos) {
      ...
      tc.value = `HORARIO - ${nombreSede} - ${nombreGrupo}`;

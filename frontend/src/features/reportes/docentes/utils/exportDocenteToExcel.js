...
const resolveLookupsForSesiones = fetchSesionesContext;

// ─── Resolver lookups desde cache bulk ──────────────────────────────────────
export const resolveLookupsFromCache = (sesiones, cache) => {
...
export const buildDocenteWorksheet = (workbook, nombreDocente, sesiones, lookups, opts = {}) => {
  ...
  for (const cluster of clusters) {
    ...
    const columns = buildColumnsForSesiones(sesCluster, cluster.bloques, programacionToGrupo, gpcToGrupo, grupoToTurno, opts.agruparDias === true);
    if (columns.length > 0) {
      clustersData.push({ cluster, columns });
      if (columns.length + 1 > maxCols) maxCols = columns.length + 1;
    }
  }
  if (clustersData.length === 0) return false;
  ...
  const eventFontStrong = { name: 'Arial', size: 11, bold: true, color: { argb: 'FF1F2937' } };
  ...
  // ── Secciones de horario (una por cluster de turnos compatibles) ──────────
  for (let ci = 0; ci < clustersData.length; ci++) {
    ...
    // Plazas participantes — texto negrita azul sobre fondo blanco
    ws.mergeCells(currentRow, 1, currentRow, maxCols);
    const plazaCell = ws.getCell(currentRow, 1);
    plazaCell.value = `${cluster.plazas.length === 1 ? 'PLAZA' : 'PLAZAS'}: ${cluster.plazasLabel}`;
        ...
        columns.forEach((col, idx) => {
          ...
          if (run) {
            if (i === run.start) {
              ...
              const [codigo, cursoV, nombreCompleto, docente, , grupo, sede] = run.key.split('|');
              const richRuns = [];
              const addRun = (text, font) => { if (text) richRuns.push({ text: `${text}\n`, font }); };
              if (opts.showCodigo !== false) addRun(codigo, eventFont);
              addRun(cursoV, eventFont);
              addRun(sede, eventFontStrong);
              addRun(grupo, eventFontStrong);
              if (opts.showNombreDocente !== false) addRun(nombreCompleto, eventFont);
              if (opts.showDocente !== false) addRun(docente, eventFont);
              if (opts.showHorario !== false) addRun(timeRange, eventFont);
              if (richRuns.length > 0) richRuns[richRuns.length - 1].text = richRuns[richRuns.length - 1].text.replace(/\n$/, '');
              c.value = richRuns.length > 0 ? { richText: richRuns } : '';
            } else {
              c.value = '';
            }
            c.fill = eventFill;
...
export const exportDocenteToExcel = async (idDocente, nombreDocente, opts = {}) => {
  try {
    ...
    const lookups = opts.lookups || await resolveLookupsForSesiones(sesiones);

...
/**
 * CursosPanel — página de gestión de cursos.
 * Tabla CURSOS + CRUD via CrudMultiLevelManager.
 */
function CursosPanel() {
  const {
    tableRecords, loading, error,
    cursosCrud, tableLevelConfigs, crudLevels,
    handleSaveSuccess
  } = useCursos();
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-x-auto">
                  <TableMultiLevelEditable
                    ...
                    formatToastMessage={(recordId, field, newValue, primaryKey, rowData, header) =>
                      `${rowData?.NOMBRE_CURSO || 'Curso'}: ${header?.label || field} → ${newValue ? 'Activo' : 'No activo'}`
                    }

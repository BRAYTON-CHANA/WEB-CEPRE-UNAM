...
export default function EstadisticasModal({ isOpen, onClose, idGrupo, grupoNombre }) {
  ...
  useEffect(() => {
    ...
    const loadEstadisticas = async () => {
      setLoading(true);
      try {
        const records = await db.executeFunction('fn_estadisticas_programacion_grupo', { p_id_grupo: Number(idGrupo) });
        setData(records || []);
      } catch (err) {
  ...
  const getEstadoColor = (estado) => {
    switch (estado) {
      ...
      case 'Excede': return 'bg-red-100 text-red-700 border-red-200';
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  ...
                  <tr className="bg-gray-50/50">
                    ...
                    <th className="px-3 py-2 text-center font-medium text-gray-500 border-b">Asig.</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {data.map((row, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-center font-medium text-blue-600">{row.HORAS_CICLO_ASIGNADAS}</td>
                      ...
                      <td className="px-3 py-3 text-center font-medium text-green-600">{row.HORAS_TOTALES_ASIGNADAS}</td>

...
export const exportPostulantes = async (records, idPeriodo) => {
  try {
    ...
    // ========================================
    const sedesUnicas = [...new Set(records.map(r => r.NOMBRE_SEDE_VACANTE || 'Sin sede').filter(Boolean))];
    const carrerasUnicas = [...new Set(records.map(r => r.NOMBRE_CARRERA || 'Sin carrera').filter(Boolean))];

    const conteo = {};
    records.forEach(r => {
      const sede = r.NOMBRE_SEDE_VACANTE || 'Sin sede';
    ...
    const totalesSede = sedesUnicas.map(sede =>
      records.filter(r => (r.NOMBRE_SEDE_VACANTE || 'Sin sede') === sede).length
    );

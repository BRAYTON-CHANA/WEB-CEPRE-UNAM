...
const CepreHeader = () => {
  ...
  const navLink = `${navLinkBase} text-slate-600 hover:text-slate-900 hover:bg-slate-100`;
  const navLinkActive = `${navLinkBase} bg-slate-100 text-slate-900`;

  const userInitials = user ? `${user.nombres?.charAt(0) || ''}${user.apellidos?.charAt(0) || ''}` : '';
  const firstName = user ? user.nombres?.split(' ')[0] : '';

  return (
    <header className="bg-white/95 backdrop-blur border-b border-slate-200/70 sticky top-0 z-50">
      <nav className="w-full px-3 sm:px-4 lg:px-5">
        <div className="flex items-center justify-between h-16">
            ...
            <div className="flex-shrink-0">
              <Link to="/" className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-lg font-bold text-[#2D366F] leading-none">
                    CEPRE
                  </span>
            ...
            {user && (
              <div ref={userMenuRef} className="relative">
                <button
                    ...
                    className="flex items-center gap-2 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 rounded-full pl-3 pr-2 py-1.5 transition-all"
                >
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 overflow-hidden">
                    ...
                    <div className="px-4 py-2 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        <span className="w-10 h-10 rounded-full bg-[#2D366F] text-white text-sm font-bold flex items-center justify-center">
                          {userInitials}
                        </span>
                        <div className="flex flex-col min-w-0">
                          ...
                          <span className="text-xs text-[#2D366F] font-medium truncate">
          ...
          <div className="md:hidden">
            <button
              onClick={toggleMenu}
              className="inline-flex items-center justify-center p-2 rounded-lg text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-200"
            >
        ...
        {isMenuOpen && (
          <div className="md:hidden border-t border-slate-100">
            <div className="px-2 py-4 space-y-2">
              <Link 
                to="/" 
                className={`block px-3 py-2 rounded-lg text-base font-medium transition-colors ${isActive('/') ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}
              >
                Inicio
              </Link>
              <Link 
                to="/configuracion" 
                className={`block px-3 py-2 rounded-lg text-base font-medium transition-colors ${isActive('/configuracion') ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}
              >
                Configuración
              </Link>
              {user && (
                <div className="border-t border-slate-100 pt-3 mt-2">
                  <div className="px-4 pb-3">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-[#2D366F] font-medium">
                          {activeRole || (roles.length > 0 ? roles[0].nombre : '')}
                        </span>
                      </div>
                    </div>
                      ...
                      {roles.map((r) => (
                        <button
                          ...
                          className={`flex items-center justify-between w-full text-left px-3 py-1.5 rounded-lg text-sm transition-colors ${
                            activeRole === r.nombre
                              ? 'bg-slate-100 text-[#2D366F] font-semibold'
                  ...
                  <div className="border-t border-slate-100">
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-2 w-full text-left px-4 py-2.5 text-base font-medium text-red-700 hover:bg-red-50 transition-colors"
                    >
                      <LogOutIcon />
                      Cerrar sesión
                    </button>

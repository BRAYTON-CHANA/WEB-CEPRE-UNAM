...
export async function createConvocatoriaConPlazas(data) {
  ...
  const plazasSanitizadas = plazas
    .filter(p => p && p.ID_CURSO != null && p.NUMERO_PLAZAS > 0)
    .map(p => {
      const modalidad = p.MODALIDAD === 'VIRTUAL' ? 'VIRTUAL' : 'PRESENCIAL';
      return {
        ID_SEDE: modalidad === 'VIRTUAL' ? null : (p.ID_SEDE != null ? Number(p.ID_SEDE) : null),
 ...
 * Si (convocatoria, modalidad, sede, curso) ya existe → NO modifica el máximo, solo crea la plaza.
 ...
 *
 * @param {Object} data - { ID_CONVOCATORIA, ID_SEDE, ID_CURSO, NUMERO_PLAZAS, MODALIDAD }
 * @returns {Promise<number>} - ID_CONVOCATORIA_CURSO.
 */
export async function addConvocatoriaCursoPlazas(data) {
  const modalidad = data.MODALIDAD === 'VIRTUAL' ? 'VIRTUAL' : 'PRESENCIAL';
  const idSede = modalidad === 'VIRTUAL' ? null : data.ID_SEDE;

...
function PostulantesHeader({
  ...
  const isVirtualSede = selectedIdSede === 'virtual';
  const sedeNombre = isVirtualSede
    ? 'Virtual'
    : (sedes.find(s => String(s.ID_SEDE) === String(selectedIdSede))?.NOMBRE_SEDE || 'Sede');
        ...
        {selectedCursoRow ? (
          ...
          <>
            <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              {isVirtualSede && (
                <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 18l-1 1h10l-1-1-.75-1M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              )}

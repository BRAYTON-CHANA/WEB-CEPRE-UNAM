...
class DatabaseManager {
  ...
  static async query(sql, ...params) {
    ...
    let finalSql = sql;
    // Reemplazar placeholders $1, $2, ... (PostgreSQL style) en un solo paso.
    // Single-pass: no re-escanea las sustituciones (evita que $10 dentro de un
    // hash bcrypt como '$2a$10$N9qo...' sea reemplazado por el parámetro 10).
    finalSql = finalSql.replace(/\$(\d+)\b/g, (match, num) => {
      const idx = parseInt(num, 10) - 1;
      return idx >= 0 && idx < safeParams.length ? safeParams[idx] : match;
    });
   ...
   * Soporta filtros como objeto { campo: valor } o array { field, op, value }
   * Operadores: =, in, not in (y variantes not-in, NOT IN), !=/neq, >, <, >=, <=,
   * is/isnull (IS NULL/TRUE/FALSE), isnot/notnull (IS NOT NULL)
   */
  static async select(table, filters = {}, fields = null) {
    ...
    const applyFilters = (q, flt) => {
      if (!flt) return q;

      if (Array.isArray(flt)) {
        for (const { field, op = '=', value } of flt) {
          if (field === undefined) continue;

          const opNorm = String(op).toLowerCase().trim();

          // 'is'/'is not' aceptan null como valor válido (IS NULL / IS NOT NULL)
          if (opNorm === 'is' || opNorm === 'isnull' || opNorm === 'is null') {
            q = q.is(field, value === undefined || value === 'null' ? null : value);
            continue;
          }
          if (opNorm === 'isnot' || opNorm === 'is not' || opNorm === 'notnull' || opNorm === 'not null') {
            q = q.not(field, 'is', value === undefined || value === 'null' ? null : value);
            continue;
          }
          ...
          if (opNorm === '=' || opNorm === 'eq') {
            ...
            if (list.length > 0) q = q.in(field, list);
          } else if (opNorm === 'not in' || opNorm === 'not-in' || opNorm === 'notin') {
            const list = Array.isArray(value) ? value : [value];
            if (list.length > 0) q = q.notIn(field, list);
          } else if (opNorm === '!=' || opNorm === 'neq' || opNorm === 'not') {
            q = q.neq(field, value);
          } else if (opNorm === '>' || opNorm === 'gt') {
            q = q.gt(field, value);
          } else if (opNorm === '<' || opNorm === 'lt') {
            q = q.lt(field, value);
          } else if (opNorm === '>=' || opNorm === 'gte') {
            q = q.gte(field, value);
          } else if (opNorm === '<=' || opNorm === 'lte') {
            q = q.lte(field, value);
          } else if (opNorm === 'ilike') {
            const pattern = value.includes('%') ? value : `%${value}%`;
            q = q.ilike(field, pattern);
          } else if (opNorm === 'like') {
            const pattern = value.includes('%') ? value : `%${value}%`;
            q = q.like(field, pattern);
          }
        }
      } else if (typeof flt === 'object') {
  ...
  /**
   * Sanitizar datos: convertir strings vacíos a null (evita errores de tipo en Postgres)
   */
  ...
  static async insert(table, data) {
    const supabase = await DatabaseManager.connect();
    const { data: result, error } = await supabase.from(table).insert(DatabaseManager.sanitizeData(data)).select();
  ...
  static async insertBatch(table, dataArray) {
    const supabase = await DatabaseManager.connect();
    const sanitized = dataArray.map(DatabaseManager.sanitizeData);
    const { data: result, error } = await supabase.from(table).insert(sanitized).select();
    if (error) throw error;
    return result;
  }
  ...
  static async update(table, id, data, idColumn = 'id') {
    const supabase = await DatabaseManager.connect();
    const { data: result, error } = await supabase.from(table).update(DatabaseManager.sanitizeData(data)).eq(idColumn, id).select();
    if (error) throw error;
    return result;
  }

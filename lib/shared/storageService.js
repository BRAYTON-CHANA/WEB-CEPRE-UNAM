 ...
 * @param {string} tipo - 'cv' o 'anexos' (subcarpeta dentro de la postulación)
 * @returns {Promise<{ path: string, filename: string, contentType: string, size: number }>}
 */
export async function uploadPostulacionFile(fileBuffer, filename, contentType, idPostulacion, tipo = 'cv') {
  const supabase = getSupabase();
  const timestamp = Date.now();
  const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
  const path = `postulaciones/${idPostulacion}/${tipo}/${timestamp}-${safeName}`;

...
const BUCKET_ASISTENCIAS = 'asistencias';
 ...
 * @param {number|string} idCorreo - ID del correo o subcarpeta (para organizar el path)
...
export async function uploadAttachment(fileBuffer, filename, contentType, idCorreo) {
  ...
  const safeDir = String(idCorreo).replace(/[^a-zA-Z0-9._/-]/g, '_');
  const path = `correos/${safeDir}/${timestamp}-${safeName}`;
...
// ============================================
// Convocatoria Documentos (biblioteca de documentos) - bucket usuarios-adjuntos
// Tabla: CONVOCATORIA_DOCUMENTOS
// ============================================
...
/**
 * Copiar un archivo dentro del bucket usuarios-adjuntos (server-side, sin descargar).
 * Usado para crear snapshots de archivos al postular.
 *
 * @param {string} sourcePath - Path origen en el bucket
 * @param {string} destPath - Path destino en el bucket
 * @returns {Promise<{ path: string, filename: string }>}
 */
export async function copyUsuarioDoc(sourcePath, destPath) {
  const supabase = getSupabase();
  const { data, error } = await supabase.storage
    .from(BUCKET_USUARIOS)
  ...
  // Extraer filename del destPath
  const filename = destPath.split('/').pop() || '';

  return {
    path: data?.path || destPath,
    filename,
  };
}

// ============================================
// Asistencias Docentes (foto evidencia) - bucket asistencias
// Subcarpeta: asistencias-docentes/{idSesion}/{timestamp}-{filename}
// ============================================

/**
 * Subir una foto evidencia de asistencia docente al bucket asistencias.
 *
 * @param {Buffer} fileBuffer - Contenido del archivo
 * @param {string} filename - Nombre original del archivo
 * @param {string} contentType - MIME type del archivo
 ...
 * @returns {Promise<{ path: string, filename: string, contentType: string, size: number }>}
 */
export async function uploadAsistenciaDocenteFile(fileBuffer, filename, contentType, idSesion) {
  const supabase = getSupabase();
  const timestamp = Date.now();
  const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
  const path = `asistencias-docentes/${idSesion}/${timestamp}-${safeName}`;

  const { data, error } = await supabase.storage
    .from(BUCKET_ASISTENCIAS)
    .upload(path, fileBuffer, {
      contentType,
      upsert: false,
    });
...
/**
 * Generar una URL firmada para ver una foto evidencia de asistencia.
 *
 * @param {string} path - Path del archivo en Storage
 * @param {number} expirySeconds - Segundos de validez (default: 3600 = 1 hora)
 * @returns {Promise<string>} URL firmada
 */
export async function getAsistenciaDocenteUrl(path, expirySeconds = 3600) {
  const supabase = getSupabase();
  const { data, error } = await supabase.storage
    .from(BUCKET_ASISTENCIAS)
    .createSignedUrl(path, expirySeconds);
...
/**
 * Eliminar una foto evidencia de asistencia del bucket asistencias.
 *
 * @param {string} path - Path del archivo en Storage
 */
export async function deleteAsistenciaDocenteFile(path) {
  const supabase = getSupabase();
  const { error } = await supabase.storage
    .from(BUCKET_ASISTENCIAS)
    .remove([path]);
...
export default {
  ...
  getUsuarioDocUrl,
  deleteUsuarioDoc,
  // Copia de archivos (snapshots)
  copyUsuarioDoc,
  // Asistencias Docentes (foto evidencia)
